...
### אימות חשבון אחרי הרשמה

- לאחר הרשמה / התחברות ללא אימות — מעבר ל־`/auth/verify` (בחירת ערוץ אימייל / SMS), ואז `/auth/verify/code` (6 ספרות).
- **Resend** (אימייל): `RESEND_API_KEY`, אופציונלי `RESEND_FROM_EMAIL`. ב־Resend לא מאפשרים דומיינים ציבוריים חינמיים (למשל `*.vercel.app`) — צריך דומיין שאתה הבעלים שלו, מאומת ב־Resend וב־DNS. בפרודקשן בלי מפתח — שליחת אימייל נכשלת (בפיתוח הקוד מודפס ללוג בשרת).
- **Twilio** (SMS): `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM_NUMBER` (פורמט בינלאומי). בלי מפתחות — בפרודקשן SMS נכשל.
- משתמשים קיימים במסד (לפני המיגרציה) מסומנים כמאומתים אוטומטית.

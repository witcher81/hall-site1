...
- לאחר הרשמה / התחברות ללא אימות — מעבר ל־`/auth/verify` (בחירת ערוץ אימייל / Gmail / SMS), ואז `/auth/verify/code` (6 ספרות).
- **Resend** (אימייל): `RESEND_API_KEY`, אופציונלי `RESEND_FROM_EMAIL`. בפרודקשן בלי מפתח — שליחת אימייל נכשלת (בפיתוח הקוד מודפס ללוג בשרת).

...
const MAX_VIEWS_PER_VENUE_PER_HOUR = 80;
...
export async function POST(req: NextRequest) {
  ...
  const recentCount = await prisma.venuePageView.count({
    where: { venueId, createdAt: { gte: hourAgo } },
  });
  if (recentCount >= MAX_VIEWS_PER_VENUE_PER_HOUR) {

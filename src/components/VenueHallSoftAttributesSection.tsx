...
const chipClass =
  "relative flex min-w-0 cursor-pointer items-center gap-2.5 rounded-xl border border-[#E7E0CF]/80 bg-white px-3 py-2.5 text-sm font-medium text-[#1A1A1A] transition hover:border-[#C9A227]/50";
const cbClass =
...
export default function VenueHallSoftAttributesSection({
  ...
  return (
    <div>
      ...
      <p className="mb-2 text-xs font-semibold text-[#5F5F5F]">הוסיפו פרט משלכם</p>
      ...
      <div className="mb-4 rounded-xl border border-[#E0D4C3]/90 bg-white/70 p-3">
        ...
        {customRows.map((row) => (
          <div key={row.id} className={`${chipClass} pe-9`}>
            ...
            <button
              type="button"
              title="הסר פריט"
              className="absolute end-2 top-1/2 -translate-y-1/2 rounded px-1 text-[11px] text-[#6B6560] hover:bg-[#EFE6D5] hover:text-[#1A1A1A]"
